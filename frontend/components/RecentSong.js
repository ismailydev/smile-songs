...
export default function RecentSong({ song }) {
    const { _id: id, title, artist, album } = song;

    return (
        <div className="relative shadow-sm rounded-l-none shadow-white/5 rounded-3xl w-full md:w-1/5 h-40 cursor-pointer opacity-75 hover:opacity-100 hover:shadow-white/10 transition-all duration-300 ease-in-out">
            <Link href={`/songs/${id}`}>
